...
\noindent La figure~\ref{fig:backend-security-chain} montre le flux : routes publiques (signup/login) sont directement acheminées, les routes protégées vérifient d'abord une clé API interne (service IA), puis un JWT utilisateur. Le JWT est validé et l'identité extraite. Toute tentative d'accès invalide retourne 401 Unauthorized.

\paragraph{Autorisation et isolation des ressources \\}

Chaque ressource (vidéo, transcription, message) est liée à un propriétaire. Une vérification d'appartenance s'assure que seul le propriétaire de la ressource peut y accéder ; accès non autorisé retourne 403 Forbidden.
