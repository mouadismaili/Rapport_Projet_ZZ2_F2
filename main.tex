...
Le projet OCULA avait pour ambition de transformer une vidéo en une ressource intelligemment exploitable. 
Au-delà de la simple transcription automatique, l’objectif était de concevoir un système capable de structurer, synthétiser et rendre interrogeable un contenu vidéo en langage naturel.

Les résultats obtenus démontrent la faisabilité de cette approche. 
Le système permet aujourd’hui :
\begin{itemize}
    \item la transcription automatique d’une vidéo via un modèle de reconnaissance vocale,
    \item la génération automatique d’un titre et d’un résumé cohérents,
    \item l’indexation sémantique du transcript via des embeddings vectoriels,
    \item l’interrogation contextualisée du contenu grâce à un mécanisme RAG,
    \item la restitution de réponses accompagnées de références temporelles explicites.
\end{itemize}

D’un point de vue pédagogique et technique, le projet a permis de consolider des compétences en architecture microservices, traitement asynchrone, intégration de modèles d’intelligence artificielle, recherche vectorielle et conception de pipelines robustes et explicables.

\subsection{Améliorations techniques possibles}

Bien que fonctionnel, le système peut être amélioré sur plusieurs axes afin d’en accroître la performance, la robustesse et les capacités.

\subsubsection{Streaming transcription}

Actuellement, la transcription est réalisée de manière batch après l’upload complet de la vidéo. 
Une évolution naturelle consisterait à intégrer un mécanisme de transcription en streaming, permettant d’indexer progressivement le contenu pendant le traitement. 
Cela réduirait la latence perçue par l’utilisateur et ouvrirait la voie à une interrogation quasi temps réel.

\subsubsection{Analyse de vidéo frame par frame}

Le périmètre actuel repose exclusivement sur le contenu audio. 
L’intégration d’une analyse visuelle (détection d’objets, OCR, reconnaissance d’éléments graphiques) permettrait d’exploiter les informations présentes uniquement à l’écran. 
Une approche multimodale combinant audio et image améliorerait significativement la richesse du système.

\subsubsection{Infrastructure}

L’architecture actuelle repose sur une séparation frontend/backend/service IA adaptée à un contexte académique. 
Pour un déploiement à plus grande échelle, plusieurs optimisations seraient envisageables :
\begin{itemize}
    \item distribution de la base vectorielle,
    \item mise en cache des requêtes fréquentes,
    \item orchestration via conteneurisation avancée,
    \item surveillance et métriques centralisées.
\end{itemize}

Ces améliorations renforceraient la scalabilité et la résilience du système.

\subsection{Ouverture produit : industrialisation}

Au-delà de son cadre académique, OCULA présente un potentiel d’industrialisation important. 
Un tel système pourrait être appliqué à des plateformes de formation en ligne, à la gestion de conférences enregistrées, à la documentation d’entreprise ou encore à l’archivage intelligent de contenus audiovisuels.

Une industrialisation impliquerait :
\begin{itemize}
    \item un renforcement des mécanismes de sécurité et de conformité,
    \item une gestion fine des coûts liés aux appels aux modèles externes,
    \item une optimisation de la performance et de la latence,
    \item une amélioration continue des modèles et des paramètres RAG.
\end{itemize}

Ainsi, OCULA constitue non seulement une démonstration technique cohérente, mais également une base solide pour le développement futur d’outils d’exploration intelligente de contenus vidéo à grande échelle.
