...
\begin{itemize}
    ...
    \item Une fois la fonctionnalité terminée, la branche est fusionnée dans \path{develop},
...
\begin{itemize}
    ...
    \item \textbf{Tests dans les conteneurs} : tests et validations effectués dans les conteneurs, assurant la compatibilité avec l'environnement de production.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Consultation de la documentation} : les développeurs consultaient la documentation officielle pour résoudre les problèmes,
...
Le backend d'OCULA est une API REST basée sur Spring Boot, responsable de l'orchestration transactionnelle du système, de la gestion des données métier et de la coordination avec les services externes (stockage objet, service IA). Il garantit l'intégrité des entités, l'authentification des utilisateurs et la notification en temps réel via WebSocket.

\subsubsection{Organisation en couches}

L'architecture du backend suit le pattern classique stratifié, où chaque couche assume une responsabilité bien définie et interagit explicitement avec ses voisines. Cette séparation des responsabilités offre plusieurs avantages : testabilité accrue (chaque couche peut être testée indépendamment), évolutivité (modification d'une couche sans impact sur les autres) et maintenabilité (logique métier centralisée).
...
\paragraph{Entités principales et cardinalités}

\begin{figure}[H]
\centering
\begin{tikzpicture}[
    entity/.style={draw, rectangle, rounded corners=4pt, minimum width=2.2cm, minimum height=0.7cm, font=\bfseries, fill=blue!8, line width=1.5pt},
    relation/.style={-latex, thick, line width=1.2pt},
    label/.style={font=\normalsize}
]

% Entités
\node[entity] (user) at (0,2) {User};
\node[entity] (video) at (3.5,2) {Video};
\node[entity] (transcript) at (3.5,0) {VideoTranscript};
\node[entity] (message) at (7,2) {Message};

% Relations
\draw[relation] (user.east) -- (video.west) node[midway, above, label]{0:N};
\draw[relation] (video.south) -- (transcript.north) node[midway, right, label]{1:N};
\draw[relation] (video.east) -- (message.west) node[midway, above, label]{1:N};

\end{tikzpicture}
\caption{Vue simplifiée des entités du domaine métier OCULA}
\label{fig:domain-entities-simple}
\end{figure}
...
\vspace{0pt}
Représente un utilisateur authentifié du système. Identifiée par un UUID, elle stocke les identifiants de connexion (email, mot de passe haché via BCrypt), le profil (prénom, nom) et optionnellement un avatar. L'email est unique et obligatoire. La relation \texttt{1:N} vers Video impose une cascade de suppression : supprimer un utilisateur supprime automatiquement toutes ses vidéos et épargne les opérations d'orphelinat. L'entité implémente \texttt{UserDetails} de Spring Security, fournissant les informations nécessaires à l'authentification.
\end{minipage}
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item \texttt{POST /auth/signup} -- Inscription. Accepte email, mot de passe et données profil. Retourne l'utilisateur persisté (200 OK) ou erreur 409 si email déjà enregistré.
        \item \texttt{POST /auth/login} -- Connexion. Accepte email et mot de passe. Retourne JWT, durée d'expiration et infos utilisateur (200 OK) ou 401 en cas de credentials invalides.
    \end{itemize}

    \item \textbf{Gestion des vidéos} (\textit{/videos})
    \begin{itemize}
        \item \texttt{POST /videos/upload} -- Upload. Accepte fichier vidéo, miniature, titre, résumé et ID utilisateur. Crée les clés blob, stocke les fichiers, crée l'entité Video en base (statut PENDING), retourne VideoDto (200 OK) ou 400/401 en cas d'erreur.
        \item \texttt{GET /videos/\{id\}} -- Lecture. Retourne les détails d'une vidéo (200 OK) ou 404 si absent.
        \item \texttt{GET /videos/page/\{userId\}} -- Liste paginée. Retourne un Page<VideoDto> (200 OK). Paramètres de query: page, size, sort.
        \item \texttt{PUT /videos/update/\{id\}} -- Mise à jour. Modifie titre, résumé ou avatar. Format multipart. Retourne VideoDto (200 OK) ou 404.
        \item \texttt{POST /videos/process/\{id\}} -- Déclenchement IA. Enclenche le pipeline de transcription asynchrone via le service Flask. Retourne statut de la tâche (202 Accepted) ou 404.
        \item \texttt{PUT /videos/update/status/\{id\}} -- Mise à jour de statut (interne). Endpoint appelé par le service IA pour signaler l'avancement (PROCESSING, COMPLETED, FAILED). Déclenche notification WebSocket. 200 OK ou 404.
    \end{itemize}

    \item \textbf{Transcripts} (\textit{/video-transcripts})
    \begin{itemize}
        \item \texttt{POST /video-transcripts} -- Persistance segment. Accepte videoId, startTime, endTime, transcriptText. Retourne l'entité créée (201 Created) ou 400 en cas de validation échouée (ex : endTime < startTime).
    \end{itemize}

    \item \textbf{Messages (Chatbot)} (\textit{/messages})
    \begin{itemize}
        \item \texttt{GET /messages/page/\{videoId\}} -- Historique paginé. Retourne les messages d'une vidéo. Paramètres de query : page, size. 200 OK ou 404 si vidéo absente.
        \item \texttt{POST /messages} -- Envoi message (future amélioration, actuellement géré par le service IA via callbacks).
    \end{itemize}

    \item \textbf{Stockage objet} (\textit{/files})
    \begin{itemize}
        \item \texttt{GET /files/get-url?key=...} -- Génération d'URL signée. Accepte la clé blob, retourne une URL sécurisée (200 OK). La clé est généralement au format \texttt{userId/videos/...} ou \texttt{userId/thumbnails/...}.
    \end{itemize}

    \item \textbf{Utilisateurs} (\textit{/users})
    \begin{itemize}
        \item \texttt{PUT /users/update/\{id\}} -- Profil. Mise à jour prénom, nom, email, mot de passe, avatar. Accepte multipart/form-data. Validation : email doit être unique (409 si collision), mot de passe doit respecter contraintes (min 8 caractères). Retourne UserDto (200 OK) ou 400/409/404.
    \end{itemize}
\end{enumerate}

\paragraph{Exemple d'endpoint représentatif : POST /videos/upload}

\begin{lstlisting}[language=Java, caption={Endpoint d'upload vidéo avec validation},label={lst:endpoint-upload}]
@PostMapping("/upload")
public ResponseEntity<VideoDto> createVideoWithUpload(
        ...
        @RequestParam("duration") String durationSeconds) 
            throws IOException, UserNotFoundException {
    
    Duration duration = Duration.ofSeconds(Long.parseLong(durationSeconds));
    String videoKey = userId + "/videos/" + UUID.randomUUID();
    String thumbnailKey = userId + "/thumbnails/" + UUID.randomUUID();
    ...
    VideoDto createdVideo = videoService.createVideoWithUpload(
        videoKey, thumbnailKey, title, summary, userId, duration);

    return ResponseEntity.ok(createdVideo);
}
\end{lstlisting}

Cet endpoint orchestre le flux : réception du fichier multipart, stockage en objet, création de l'enregistrement métier. Le service valide l'existence de l'utilisateur et génère les clés blob de manière sécurisée.

\paragraph{Exemple d'endpoint : POST /auth/login}

\begin{lstlisting}[language=Java, caption={Endpoint d'authentification retournant JWT},label={lst:endpoint-login}]
@PostMapping("/login")
...
Le service d'authentification (\textit{AuthenticationService}) effectue la vérification du mot de passe. En cas d'erreur, une exception \textit{InvalidParamsException} est levée et mappée à 401 Unauthorized par le gestionnaire global d'exceptions.
...
\noindent \textit{À développer -- Stratégie JWT stateless, filtres de sécurité, gestion des tokens, certificats et rotations.}

\subsubsection{Extraits de code importants}

Cette section illustre les patterns clés du backend à travers des exemples courts et commentés.

\paragraph{Repository JPA typé}

\begin{lstlisting}[language=Java, caption={Repository JPA pour accès aux vidéos},label={lst:repo-video}]
@Repository
...
Spring Data JPA génère automatiquement les implémentations SQL pour \texttt{findAllByUserId}, optimisant les requêtes paginées. La pagination côté base de données limite la transmission réseau.

\paragraph{Entité avec cascade et relations}

\begin{lstlisting}[language=Java, caption={Entité User avec relation OneToMany en cascade},label={lst:entity-user}]
@Entity
@Table(name = "Users")
...
L'annotation \texttt{CascadeType.REMOVE} assure que supprimer un utilisateur supprime implicitement toutes ses vidéos, évitant l'incohérence relationnelle.

\paragraph{Validation avec contraintes métier}

\begin{lstlisting}[language=Java, caption={Entité VideoTranscript avec validation},label={lst:entity-transcript}]
@Entity
@Table(name = "VideoTranscripts")
public class VideoTranscript {
    ...
    @NotBlank(message = "Transcript text is mandatory")
    @Column(columnDefinition = "TEXT", nullable = false)
    private String transcriptText;
    
    // Validation métier : endTime >= startTime effectuée au niveau service
}
\end{lstlisting}

Les constraints JPA (@NotBlank, @Email, etc.) valident les invariants simples. Les règles métier complexes (ex : endTime >= startTime) sont validées dans le service.

\paragraph{Configuration Spring Data et OpenAPI}

\begin{lstlisting}[language=Java, caption={Configuration Enable OpenAPI et Swagger pour documentation},label={lst:config-openapi}]
@Configuration
@EnableOpenApi
public class OpenApiConfiguration {
    ...
    public OpenAPI customOpenAPI() {
        return new OpenAPI()
            .info(new Info().title("OCULA API")
                .version("1.0.0")
                .description("API REST du backend OCULA"));
    }
}
\end{lstlisting}

Cette configuration expose automatiquement la documentation Swagger à \texttt{/swagger-ui.html} et génère le schéma OpenAPI à \texttt{/v3/api-docs}, facilitant l'intégration frontend et la documentation.
...
Pour diffuser une vidéo dans notre application Ocula il faudrait obtenir:
...
Ainsi, OCULA constitue non seulement une démonstration technique cohérente, mais également une base conceptuelle solide pour le développement futur d’outils d’exploration intelligente de contenus vidéo à grande échelle.
