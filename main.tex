...
\titre{OCULA : Application Web pour l’Analyse Intelligente des Contenus Vidéo}
...
Nous souhaitons adresser nos sincères remerciements à toutes les personnes qui ont accompagné et soutenu la réalisation de ce projet, contribuant ainsi à son aboutissement.\vspace{0.2cm}

Nous exprimons tout particulièrement notre gratitude à notre tuteur, \textbf{M. Loïc Yon}, pour la qualité de son encadrement, sa disponibilité constante et la pertinence de ses conseils tout au long de ce travail. Son expertise et ses remarques constructives ont joué un rôle essentiel dans la réussite de ce projet.\vspace{0.2cm}

Nous remercions également chaleureusement \textbf{Mme Aurélie Aubert}, notre professeure de communication, pour son accompagnement attentif et ses recommandations précieuses. Son enseignement nous a permis d’améliorer l’organisation de nos idées et d’apporter rigueur et clarté à la rédaction de ce rapport.\vspace{0.2cm}

Enfin, nous tenons à remercier l’ensemble des personnes ayant contribué, de près ou de loin, à ce projet. Par leurs conseils, leur expertise ou leur soutien, chacune a participé à la concrétisation de ce travail, qui demeure avant tout le fruit d’un effort collectif.
...
Le présent projet s’inscrit dans le cadre du projet de deuxième année de l’Institut Supérieur d’Informatique, de Modélisation et de leurs Applications (ISIMA), filière Génie Logiciel et Systèmes Informatiques. 
Il vise à concevoir et implémenter un système logiciel complet intégrant des techniques modernes d’intelligence artificielle au sein d’une architecture web full-stack.

À l’ère du numérique, la vidéo constitue un support majeur de diffusion de connaissances, qu’il s’agisse de cours enregistrés, de conférences, de tutoriels techniques ou de réunions professionnelles. 
Cependant, contrairement au texte, une vidéo ne se parcourt pas efficacement : retrouver une information précise nécessite souvent un visionnage long et séquentiel. 
Même lorsque des transcriptions sont disponibles, celles-ci restent volumineuses et peu structurées, rendant la recherche, la synthèse et l’exploitation du contenu complexes.

Les avancées récentes en reconnaissance automatique de la parole et en modèles de langage permettent d’envisager une approche nouvelle : transformer une vidéo en une ressource interrogeable en langage naturel. 
L’enjeu ne se limite plus à produire un simple transcript, mais consiste à structurer le contenu, générer des métadonnées pertinentes et permettre une interaction contextualisée tout en garantissant la traçabilité des réponses.

C’est dans ce contexte qu’a été développé OCULA, une application web reposant sur un frontend Angular, un backend Java Spring Boot et un microservice d’intelligence artificielle en Flask. 
Le système permet l’upload de vidéos, leur transcription automatique, la génération d’un titre et d’un résumé, l’indexation sémantique du contenu et l’interrogation via un mécanisme Retrieval-Augmented Generation (RAG). 
Les réponses produites sont accompagnées de références temporelles, assurant explicabilité et vérifiabilité.

La problématique centrale du projet peut ainsi être formulée comme suit : comment transformer automatiquement une vidéo longue en une ressource structurée, synthétisée et interrogeable en langage naturel, tout en garantissant la pertinence et la traçabilité des réponses ?

Pour répondre à cette problématique, une architecture distribuée intégrant transcription automatique, génération de métadonnées, indexation vectorielle et interaction par modèle de langage a été conçue et implémentée. 
Le projet mobilise ainsi des compétences en architecture logicielle, intégration de services d’intelligence artificielle, gestion de traitements asynchrones et conception de pipelines robustes.

Le présent rapport est organisé de manière progressive : le chapitre 2 présente l’organisation du projet et la méthodologie adoptée ; le chapitre 3 expose les fondements théoriques et l’état de l’art ; le chapitre 4 détaille les choix de conception et l’architecture ; le chapitre 5 décrit l’implémentation technique ; le chapitre 6 analyse les résultats obtenus ; le chapitre 7 aborde les considérations de sécurité et d’éthique ; enfin, le chapitre 8 conclut le travail et propose des perspectives d’évolution.\section{Organisation et gestion du projet}
...
\begin{figure}[H]
    ...
    \caption{Importation d'une vidéo}
    \label{fig:placeholder}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Traitement d'une vidéo}
    \label{fig:placeholder}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Mécanisme de réponse de l'assistant IA}
    \label{fig:placeholder}
\end{figure}
...
Pour chaque vidéo, une collection dédiée (\path{video_<video_id>}) est utilisée ou créée lors de l’initialisation du flux d’indexation.
...
\begin{itemize}
    ...
    \item des métadonnées : \path{video_id}, \path{start_time}, \path{end_time}.
\end{itemize}
\begin{figure}[H]
    \centering
...
Le modèle relationnel assure la cohérence transactionnelle des données métier, tandis que la couche vectorielle optimise la récupération contextuelle pour le RAG ; les deux couches sont donc complémentaires et synchronisées par \path{video_id}.
\begin{figure}[H]
    \centering
...
Chaque vidéo dispose d’une collection nommée \path{video_<video_id>}. 
...
Ces trois couches sont complémentaires et interconnectées principalement par \path{video_id} (et \path{user_id} pour les ressources utilisateur). 
\medskip
...
L'architecture du backend suit le pattern classique en couches, où chaque couche assume une responsabilité bien définie et interagit explicitement avec ses voisines.
...
\begin{itemize}
    ...
    \item \textbf{Couche persistance (Repositories)} : contrôle l'accès aux données via Spring Data JPA, masquant la complexité SQL.
...
Le modèle de données d'OCULA repose sur quatre entités principales structurant les domaines clés : utilisateurs, vidéos, transcriptions et messages. Chaque entité est persistée en base relationnelle via JPA et possède des responsibilities explicites dans le cycle de traitement des vidéos.
...
\vspace{0pt}
Représente un utilisateur authentifié du système. Identifiée par un UUID, elle stocke les identifiants de connexion (email, mot de passe haché), le profil (prénom, nom) et optionnellement un avatar.
\end{minipage}
...
\begin{minipage}[t]{0.62\textwidth}
\vspace{0pt}
Entité centrale du système, modélisant une vidéo téléversée. Elle maintient des références vers ses ressources (contenu vidéo, miniature) via des clés de stockage objet, et inclut les métadonnées générées (titre, résumé). Le champ \texttt{status} (énumération : PENDING, PROCESSING, COMPLETED, FAILED) orchestre explicitement le cycle de vie asynchrone du traitement IA. Cela permet au frontend de suivre l'avancement.
\end{minipage}
...
\begin{minipage}[t]{0.62\textwidth}
\vspace{0pt}
Stocke un segment temporel de la transcription.
\end{minipage}
...
\begin{minipage}[t]{0.62\textwidth}
\vspace{0pt}
Enregistre chaque échange du chatbot avec l'utilisateur, y compris le rôle de l'émetteur (énumération : USER ou BOT).
\end{minipage}
...
\noindent La figure~\ref{fig:backend-security-chain} montre d'abord la phase d'authentification où les routes publiques (signup/login) sont directement acheminées, tandis que les routes protégées vérifient d'abord une clé API interne (service IA), puis un JWT utilisateur. Une fois le JWT validé et l'identité extraite, la requête passe à la phase d'autorisation où l'identité authentifiée est utilisée pour contrôler l'accès aux ressources métier. Chaque ressource (vidéo, transcription, message) est liée à un propriétaire. Une vérification d'appartenance s'assure que seul le propriétaire de la ressource peut y accéder.
\subsubsection{Extraits de code importants des services}

Cette section se concentre sur deux services backend clés pour l'intégration IA et la communication temps réel : \texttt{AiService} et \texttt{WebsocketNotifier}.

\paragraph{Service d'intégration IA \\}

Le code ci-dessous fait le lien entre le backend Spring Boot et le service IA Python pour le traitement d'une vidéo et les requêtes de chat.
...
\noindent
\textbf{Description :} Ce code fait la construction des requêtes HTTP vers \texttt{/process} et \texttt{/chat}, puis récupère les réponses du service IA pour alimenter le flux métier du backend.

\paragraph{Notification temps réel via WebSocket \\}

Le code ci-dessous fait la diffusion des changements de statut vidéo vers les clients connectés en temps réel.

\begin{lstlisting}[language=Java, caption={WebsocketNotifier : diffusion des changements de statut vidéo},label={lst:websocket-notifier}]
@Service
...
\noindent
\textbf{Description :} Ce code fait la publication des événements de statut sur \texttt{/topic/videos/\{videoId\}}. Les clients abonnés reçoivent immédiatement les mises à jour de progression (PENDING, PROCESSING, COMPLETED).

\subsection{Service IA : LangChain + Flask}
Le traitement intelligent des vidéos est assuré par un service dédié développé en Python et exposé via Flask. 
...
La vidéo est d’abord téléchargée depuis Azure Blob Storage à l’aide de sa clé d’objet (\path{object_key}) vers un fichier temporaire local. 
La transcription est ensuite effectuée via \path{whisper_model.transcribe}, qui retourne :
...
Chaque segment est représenté par une borne \path{end_char} et ses timestamps \path{start_end}. 
Ensuite, pour chaque chunk extrait, sa position \path{[chunk_start, chunk_end]} dans le texte global est retrouvée, puis convertie en timestamps en recherchant les segments couvrant ces bornes.
...
Dans l’implémentation actuelle, les embeddings ne sont pas normalisés lors de l’encodage (\path{normalize_embeddings=False}), et l’exécution est configurée sur CPU.
...
(i) le texte du chunk (\path{page_content}), et (ii) les métadonnées nécessaires à la traçabilité : \path{video_id}, \path{start_time} et \path{end_time}. 
...
Les documents vectoriels sont stockés dans ChromaDB via l’interface \path{langchain_chroma.Chroma}. 
L’indexation est organisée par vidéo : chaque vidéo possède une collection dédiée nommée \path{video_<video_id>}. 
